...
export function normalizeParentRequests(rows: Row[]): ParentRequest[] {
  return rows
    ...
    .map((row) => ({
      ...
      dedicatedHoursFromExport: parseCsvNumber(row['Tiempo dedicado']) || undefined,
      totalDedicatedHoursFromExport: parseCsvNumber(row['Tiempo total dedicado']) || undefined,
...
export function normalizeTimeEntries(rows: Row[]): TimeEntry[] {
  return rows.map((row, index) => {
    ...
    const user = cleanText(row['Usuario'])

...
<template>
  ...
  >
    <Column field="parentProject" header="Proyecto padre" sortable style="min-width: 200px" />
    ...
    <Column field="estimatedHours" header="Estimadas" sortable style="width: 120px">
      <template #body="{ data }">{{ fmt(data.estimatedHours) }}</template>
    </Column>

    <Column field="actualHours" header="Reales imputadas" sortable style="width: 140px">
      <template #body="{ data }">{{ fmt(data.actualHours) }}</template>
    </Column>

    <Column field="differenceHours" header="Diferencia" sortable style="width: 120px">
      <template #body="{ data }">
    ...
    <Column field="deviationPercent" header="Desviación %" sortable style="width: 120px">
      <template #body="{ data }">{{ fmt(data.deviationPercent) }}%</template>
    </Column>

    <Column field="resultStatus" header="Resultado" sortable style="width: 120px">
      <template #body="{ data }">
    ...
    <Column field="peopleCount" header="Nº usuarios" sortable style="width: 120px" />

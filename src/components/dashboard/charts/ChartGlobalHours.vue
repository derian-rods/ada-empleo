...
interface ChartGlobalHoursProps {
  requests: CalculatedRequest[];
}

const props = defineProps<ChartGlobalHoursProps>();
const chartRef = ref();

// Prepare data for global hours comparison - ALL requests sorted by estimated hours
const chartData = computed(() => {
  const sortedRequests = [...props.requests]
    .sort((a, b) => (b.estimatedHoursTotal || 0) - (a.estimatedHoursTotal || 0))
    .map((r) => ({
      code: r.code,
      subject: r.subject,

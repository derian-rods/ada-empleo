...
function makeRequest(overrides: Partial<CalculatedRequest> = {}): CalculatedRequest {
  return {
    parentId: '1000',
    code: '1000',
    subject: 'Test',
...
describe('calculateDashboardSummary', () => {
  it('calculates totals correctly', () => {
    const requests = [
      makeRequest({ estimatedHours: 100, actualHours: 80, resultStatus: 'profit' }),
      makeRequest({
        parentId: '1001',
        estimatedHours: 50,
        actualHours: 70,
  ...
  it('calculates average deviation only from requests with estimated > 0', () => {
    const requests = [
      ...
      makeRequest({ parentId: '1002', estimatedHours: 200, deviationPercent: 10 }),
    ]

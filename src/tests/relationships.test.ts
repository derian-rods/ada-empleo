...
describe("buildCalculatedRequests", () => {
  ...
  it("calculates estimated HBS from child profile estimates", () => {
    const profiledChild: ChildRequest = {
      ...child,
      estimatedHours: 10,
      estimatedHoursJp: 10,
      estimatedHoursCs: 10,
      estimatedHoursAf: 10,
      estimatedHoursAsEs: 10,
      estimatedHoursApTs: 10,
      estimatedHoursP: 10,
    };
    ...
    expect(calculatedRequests[0].estimatedHbs).toBeCloseTo(75.2);
  });

  it("calculates consumed HBS from time entry profile", () => {
    ...
    expect(calculatedRequests[0].consumedHbs).toBeCloseTo(14.9);
  });

  it("falls back to parent estimated hours when no children", () => {
    const te = makeTimeEntry({ petitionId: "1000", hours: 30 });
    const { calculatedRequests } = buildCalculatedRequests([parent], [], [te]);

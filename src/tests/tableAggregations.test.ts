...
import type { ParentRequest, ChildRequest, TimeEntry, CalculatedRequest } from '../domain/types'
...
describe('tableAggregations', () => {
  describe('buildUserTableRows', () => {
    it('creates rows per unique user', () => {
      const rows = buildUserTableRows(calculatedRequests, children, timeEntries)
  ...
  describe('buildChildRequestTableRows', () => {
    it('creates row per child request', () => {
      const rows = buildChildRequestTableRows(parents, children, calculatedRequests, timeEntries)
      expect(rows).toHaveLength(2)
    })
    ...
    it('handles missing parent gracefully', () => {
      ...
      const rows = buildChildRequestTableRows(parents, [childNoParent], calculatedRequests, [])
